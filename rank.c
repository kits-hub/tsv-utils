...
void kh_int_destroy(khash_t(32) *h);
...
KSORT_INIT(rank, khint_t, khint_lt)
...
int rank_main(int argc, char *argv[]){
    ...
    while ((c = getopt(argc, argv, "r:me:")) >= 0) {
    ...
    if ( optind == argc || argc != optind + 1 ) {
        fprintf(stderr, "\n");
        fprintf(stderr, "Usage: tsv-utils rank [options] <tsv>\n\n");
        fprintf(stderr, "Options:\n");
        fprintf(stderr, "         -r  INT top catalogs.\n");
        fprintf(stderr, "         -m      merge remain to others catalog.\n");
        fprintf(stderr, "         -e  STR extend specified string to Others catalog.\n\n");
        return 1;
    ...
    for (i = 0; i < N; ++i){
        ...
        for (j = 0; j < n; ++j){
            ...
            if( t != kh_end(h)){
                ...
                if(R > 0){
                    if( ext.l > 0 && strncmp(kh_val(h, t), ext.s, ext.l) == 0 ){
